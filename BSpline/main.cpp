...
void generateCylinder(int n) {
	...
	for (int i = 0; i <= n; ++i) {
		...
		for (int j = 0; j <= n; ++j) {
			double height = j;
			row.emplace_back(height, radius * sin(radian), radius * cos(radian));
		}
		surfaceDataPoints.emplace_back(row);
	}
}
...
void drawSurface(int p, int q) {
	...
	// controlPoints
	glBindVertexArray(vao[1]);
	std::vector<vec3> controlPoints(m * n);
	for (int i = 0; i < m; ++i) {
		for (int j = 0; j < n; ++j) {
			controlPoints[i * n + j] = bspS->controlPoints[i][j];
		}
	}
	...
	for (double u = 0; u < 1 + err; u += step) {
		for (double v = 0; v < 1 + err; v += step) {
			surfacePoints.emplace_back((*bspS)(u, v));
		}
		count++;
	}
...
void drawCurve(int p) {
	...
	for (int i = 0; i <= n; i++) {
		controlPoints[i] = bspC->controlPoints[i];
	}
	glBindBuffer(GL_ARRAY_BUFFER, vbo[1]);
	glBufferData(GL_ARRAY_BUFFER, controlPoints.size() * sizeof(vec3), controlPoints.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vec3), (void*)0);
	...
	for (double u = 0; u < 1 + err; u += step) {
		curvePoints.emplace_back((*bspC)(u));
	}
	glBindBuffer(GL_ARRAY_BUFFER, vbo[2]);
